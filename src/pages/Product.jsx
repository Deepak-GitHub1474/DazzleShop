...
function Product() {
  ...
  // set item and purchage source in local storage
  function setItemToLocalStorage() {
    localStorage.setItem("purchaseSource", "currentItem");
    localStorage.setItem("currentItem", JSON.stringify(productDetails));
    navigate("/payment");
  }
    ...
    <PageLayout>
      <div className="flex items-center justify-around flex-wrap gap-8 my-10 mx-8 min-h-[65vh]">
        <div className="border-[0.5px] border-[#e4e3e3] rounded-lg  p-2 bg-[#ffffff]">
          {wishList ? (
            <div
              ...
              className="w-10 h-10 rounded-full border-[0.5px] border-[#e4e3e3] shadow flex items-center justify-center"
            >
              <BsFillHeartFill
                size={"24px"}
                className=" absolute cursor-pointer text-[#ff0000]"
              />
            </div>
          ) : (
            <div
            ...
            >
              <AiOutlineHeart
                size={"24px"}
                className=" absolute cursor-pointer text-[#388e3c]"
              />
            </div>
          )}
        ...
        <div className="flex flex-col justify-center gap-3 ">
          <h3 className="sm:text-2xl text-xl font-semibold sm:max-w-[350px] max-w-[265px]">
          ...
          <p className=" sm:text-lg text-[#515050] sm:max-w-[600px]">
          ...
          <div className="flex items-center gap-6">
            <Link to="/payment">
              <button
                className="bg-[#388e3c] text-white w-[140px] py-2 px-2 rounded-[2px] text-base font-semibold cursor-pointer hover:bg-[#fb641b]"
            ...
            <button
              onClick={() => addToCart(productDetails.id - 1)}
              className="bg-[#8bc34a] text-white w-[140px] py-2 px-2 rounded-[2px] text-base font-semibold cursor-pointer hover:bg-[#fb641b]"
            >
              ADD TO CART
            </button>
          </div>
